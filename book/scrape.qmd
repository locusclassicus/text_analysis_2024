...
```{r warning=FALSE}
toc <- html |> 
  html_elements("#mw-content-text li , i")
...
libri_text <- map(urls, get_text_polite)
```
...
- В функции `get_text_polite` каждому URL соответствует своя `bow()`-сессия.
- Такой подход бережно относится к ресурсу и снижает риск блокировки.
