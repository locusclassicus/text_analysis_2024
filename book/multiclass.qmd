...
plan(sequential)
# i 1 of 2 tuning:     tfidf_svm
# ✔ 1 of 2 tuning:     tfidf_svm (23.5s)
# i 2 of 2 tuning:     tfidf_rf
# ✔ 2 of 2 tuning:     tfidf_rf (27.5s)
# 51.832 sec elapsed
```
...
На очень разреженных данных мы смогли немного улучшить предсказания, однако другой способ может заключаться в том, чтобы снизить размерность за счет PCA или эмбеддингов. Про эмбеддинги пойдет речь дальше, а здесь рассмотрим пример препроцессинга с использованием PCA. Это не всегда дает хороший результат.
...
```{r echo=FALSE, eval=FALSE}
save(rf_pca_tune, file = "../data/rf_pca_tune.Rdata")
```

```{r echo=FALSE}
load("../data/rf_pca_tune.Rdata")
```
