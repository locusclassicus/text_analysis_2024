...
tm_shape(roman_map) +
  ...
  tm_scalebar(breaks = c(0, 100, 200), text.size = 1, position = c("right", "top")) 
```
...
tm_shape(roman_map) +
  ...
  tm_bubbles(
    size = "capacity",
    size.scale = tm_scale_continuous(values = c(0.5, 2)),  # Настройка масштаба
    fill = "prov.type", 
...
Фоновые карты для Рима (были) доступны на сайте <https://dh.gu.se/dare/>. Но теперь <https://dh.gu.se/tiles/imperium/{z}/{x}/{y}.png> не работает.
...
tm_shape(amph_points) +
  tm_tiles("Stadia.StamenWatercolor") +
  tm_bubbles(
    size = "capacity",
    col = "white",
    border.col = "tomato",
