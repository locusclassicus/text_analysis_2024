# LLM в обработке и анализе данных
...
Для работы может понадобиться зайти в настройки аккаунта и выбрать "Enable free endpoints that may publish prompts".
...
Для работы с облачными моделями

![](images/ollama_settings.png)
...
chat$chat(
  pdftools::pdf_text("../files/gettier.pdf"),
  "Резюмируй в одном предложении статью Э. Геттиера."
  )
...
get_summary_pdf <- function(path, user_prompt) {
...
```{r eval=FALSE}
get_summary_pdf("../files/gettier.pdf", user_prompt)
