...
Нейросеть с одним скрытым слоем в нашем случае показала себя хуже, чем опорные векторы. Напомним, что нам удалось добиться увеличения $R^2$ до `r collect_metrics(svm_rs)$mean[2] |> round(3)`. Возможно, получится немного улучшить этот результат.


## Удаление стопслов

Сначала изменим рецепт приготовления данных.
